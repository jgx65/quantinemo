...
//{
//#ifdef _DEBUG
//	message("\nParamManager::check_parameter_names...\n");
//#endif
...
//	vector< string > removeIndex;
//	vector< string >::iterator iterVec;
//	string generalName, token, tail;
//	int cur_number, number;
//	unsigned int length;
//	string::size_type pos;
//
//	reset_params();     // all parameters are reseted ("set" is set to false)
//
...
//		if(input->second[0] == "NOT_SET") continue;
//		set = false;
//		generalName = input->first;
//		// cout << endl << generalName;   // debugging
//		// find the corresponding parameter set
//		for(cur_paramset = _allParams.begin(); cur_paramset != _allParams.end(); ++cur_paramset){
//			// cout << endl << "  " << (*cur_paramset)->getName();       // debugging
//			set |= (*cur_paramset)->set_param(generalName, *input->second.begin()); //is true if at least one paramset could set the param
//		}
//		if(set) continue;
//
...
//				set |= (*cur_paramset)->set_param(generalName, *input->second.begin()); //is true if at least one paramset could set the param
//			}
//			if(set) removeIndex.push_back(generalName);                    // if the given "_1" parameter is needed remove its index
//		}
//		if(set) continue;
//
...
//			for(cur_number=number; cur_number>0; --cur_number){					// retest of its own (due to NOT_SET argument)
//				string tt = generalName+"_"+toStr(cur_number);
//				input = inputParams.find(generalName+"_"+toStr(cur_number));
//				if(input != inputParams.end()) break;  // is this parameter set?
//			}
//			if(cur_number == 0) input = inputParams.find(generalName);          // not yet found try the general parameter
//			if(input == inputParams.end()) continue;                            // no parameter set for this parameter: stop here
//			if(input->second[0]=="NOT_SET") continue;
//
//			// we have got the corresponding parameter: set the general parameter
//			cur_param->second->set(*input->second.begin(), *cur_paramset);
//
...
//	}
//
//	// remove index of all "_1" if needed
//	for(iterVec = removeIndex.begin(); iterVec != removeIndex.end(); ++iterVec){
//		input = inputParams.find(*iterVec+"_1");
//		assert(input != inputParams.end());
//		inputParams[*iterVec] = input->second;  // add the parameter without index to the map
//		inputParams.erase(input);				        // remove the parameter with index
//	}
//
...
// */
//void
//ParamManager::check_parameter_names(map<string, string>& inputParams)
//{
//#ifdef _DEBUG
//	message("\nParamManager::check_parameter_names...\n");
//#endif
...
//		for(cur_paramset = _allParams.begin(); cur_paramset != _allParams.end(); ++cur_paramset){
//			// cout << endl << "  " << (*cur_paramset)->getName();       // debugging
//			set |= (*cur_paramset)->set_param(generalName, input->second); //is true if at least one paramset could set the param
//		}
//		if(set) continue;
//        
//		// if the parameter has an index 1 then remove it and retry
