...
void FindMaximum2::Fit(ColumnVector& Theta, int n_it)
{
	...
	enum State {
		Start, Restart, Continue, Interpolate, Extrapolate, Fail, Convergence
	};
...
void NonLinearLeastSquares::Value(const ColumnVector& Parameters, bool, Real& v,
		bool& oorg)
{
	Tracer tr("NonLinearLeastSquares::Value");
	Y.resize(n_obs);
	X.resize(n_obs, n_param);
...
bool NonLinearLeastSquares::NextPoint(ColumnVector& Adj, Real& test)
{
	...
	QRZ(X, U);
	QRZ(X, Y, M);     // do the QR decomposition
	test = M.SumSquare();
	cout << " " << setw(15) << setprecision(10) << test << " "
...
void NonLinearLeastSquares::GetHatDiagonal(DiagonalMatrix& Hat) const
{
	Hat.resize(n_obs);
	for (int i = 1; i <= n_obs; i++)
		Hat(i) = X.Row(i).SumSquare();
}

// the MLE_D_FI routines

void MLE_D_FI::Value(const ColumnVector& Parameters, bool wg, Real& v,
		bool& oorg)
{
	Tracer tr("MLE_D_FI::Value");
	if (!LL.IsValid(Parameters, wg)) {
		oorg = true;
		return;
	}
	v = LL.LogLikelihood();
